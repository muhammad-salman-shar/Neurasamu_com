...
(function mindGraph(){
  ...
  const PAPERS = [
    ...
    { id:'smg', code:'SMG', fx:.70, fy:.24, status:'Architecture Blueprint v2.0 · Published',
      title:'Synapse AI Memory Graph (SMG v2)',
      abstract:'An architectural specification for episodic, pointer-virtualized long-term memory in large language models. SMG v2 decouples conversation persistence from active generation through three pillars: an L1 Active Buffer (strict 4-turn sliding window), an L2 Keyword Box (semantic index with unique cryptographic IDs), and an L2 Chat Store (encrypted virtualized vault for raw payloads). Demand-paged retrieval loads only the exact historical micro-context required at inference time — achieving infinite conversational memory without token overhead or cross-domain hallucination.',
      tags:['pointer virtualization','episodic memory','sliding window','infinite context'] },
  ...
  const CONCEPTS = [
    ...
    { id:'activebuf',   label:'active buffer',        fx:.94, fy:.10 },
    { id:'keywordbox',  label:'keyword box',          fx:.92, fy:.36 },
    { id:'chatstore',   label:'chat store',           fx:.62, fy:.06 },
    { id:'pointer',     label:'pointer virtualization',fx:.95, fy:.52 },
    { id:'pagerecall',  label:'demand paging',        fx:.74, fy:.08 },
  ...
  const EDGES = [
    ...
    ['smg','activebuf'],['smg','keywordbox'],['smg','chatstore'],['smg','pointer'],['smg','pagerecall'],['smg','memory'],['smg','agi'],['smg','safety'],['smg','perception'],
